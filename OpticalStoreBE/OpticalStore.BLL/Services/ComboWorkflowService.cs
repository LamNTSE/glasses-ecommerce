...
public sealed class ComboWorkflowService : IComboWorkflowService
{
    private readonly OpticalStoreDbContext _dbContext;

    public ComboWorkflowService(OpticalStoreDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<object> CreateAsync(ComboUpsertDto request, CancellationToken cancellationToken = default)
    {
        ...
        return await BuildComboResponse(combo.Id, cancellationToken);
    }

    public async Task<object> UpdateAsync(string comboId, ComboUpsertDto request, CancellationToken cancellationToken = default)
    {
        ValidateComboRequest(request);
    ...
    public async Task<object> UpdateStatusAsync(string comboId, string status, CancellationToken cancellationToken = default)
    {
        var combo = await _dbContext.Combos.FirstOrDefaultAsync(x => x.Id == comboId && !(x.IsDeleted ?? false), cancellationToken);
        if (combo is null)
        {
        ...
        if (string.Equals(status, "ACTIVE", StringComparison.OrdinalIgnoreCase) && combo.EndTime < DateTime.UtcNow)
        {
            throw new AppException("COMBO_CANNOT_ACTIVATE_EXPIRED", "Cannot activate expired combo.", HttpStatusCode.BadRequest);
        }

        combo.Status = status.ToUpperInvariant();
    ...
    public async Task<PagedResultDto<object>> GetAllAsync(
        string? status,
        DateTime? fromDate,
        DateTime? toDate,
        int page,
    ...
    public async Task<object> GetByIdAsync(string comboId, CancellationToken cancellationToken = default)
    {
        return await BuildComboResponse(comboId, cancellationToken);
    }

    public async Task<List<object>> GetAvailableAsync(DateTime? currentTime, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<object> ValidateAsync(ComboValidateDto request, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var comboItem in combo.ComboItems)
        {
            if (comboItem.ProductVariantId is not null)
            {
                ...
                {
                    return new { isValid = false, reason = "NOT_MATCH_RULE" };
                }
            }
        }
        ...
        if (!stock.isAvailable)
        {
            return new { isValid = false, reason = "OUT_OF_STOCK" };
        }

        var discount = CalculateComboDiscount(combo, combo.ComboItems);
        return new { isValid = true, discountAmount = discount };
    }

    public async Task<object> CheckStockAsync(string comboId, CancellationToken cancellationToken = default)
    {
        return await CheckComboStockInternal(comboId, cancellationToken);
    }

    public async Task DeleteAsync(string comboId, CancellationToken cancellationToken = default)
    {
        var combo = await _dbContext.Combos.FirstOrDefaultAsync(x => x.Id == comboId && !(x.IsDeleted ?? false), cancellationToken);
        if (combo is null)
        {
    ...
    private async Task<List<ComboItem>> BuildComboItems(string comboId, List<ComboItemDto>? requests, CancellationToken cancellationToken)
    {
        var result = new List<ComboItem>();

        foreach (var request in requests ?? new List<ComboItemDto>())
        {
    ...
    private static void ValidateComboRequest(ComboUpsertDto request)
    {
